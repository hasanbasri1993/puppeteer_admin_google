...
**Batch processing for turn-off requests.** `controllers/turnOff.js` looks up NIS (student ID) → `ID_GOOGLE` mappings from `ids.json`, chunks them into batches (`BATCH_SIZE`/`BATCH_DELAY` env vars) to avoid overloading the single shared browser instance, and pushes progress to the frontend over Pusher (channel `turn_off`, event `status-update`) as each batch/user completes.
